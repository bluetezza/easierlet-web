...
**Deployment infrastructure addendum (same day, second session):** `feature_flags` table (migration `add_deployment_infrastructure`, rollback in `backups/`), `_shared/flags.ts` + `ELFeatureFlags.swift` + `ELP.loadFlags()` with identical semantics on all three surfaces (enabled=false → off; pct=0 → `enabled_for` beta list only — fixed a hole in the prompt's draft where pct=0 returned enabled for everyone; stable djb2 hash for cohorts). All six AI flags seeded **disabled**; every AI edge function, plus tenant chat start, listing AI controls, helper bubbles, report generation, and the dashboard digest now gate on them. ai-maintenance drains active conversations to the landlord if the flag is killed mid-chat. Admin: `/flags.html` (toggles, rollout slider, beta UUIDs, **Disable All AI** kill switch) + admin-api `get_flags`/`update_flag`/`disable_all_ai`. `DEPLOY.md` authored at repo root (the prompt's companion file wasn't in the delivery bundle), `backups/` with schema snapshot (catalog JSON — full `supabase db dump` needs Docker running) + rollback script, `.gitignore` blocks `data_*.sql`. NOTE: production has no landlord account yet (only admin@easierlet.com), so no user is on any beta list — when Terry's landlord account exists, enable flags for it via admin → Flags.
